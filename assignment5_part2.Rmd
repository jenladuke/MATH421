...
```{r}
df %>% 
  summarise(SEX, na.rm=NA)
```
There is 80,587 NAs


3. Still with variable `SEX`. There are missing values in this variables that are not `NAs`. Identy the forms of missing values in this variable. Change all the forms of missing values to `NAs`. 
```{r}
...
```{r}
str_replace(df$TRAV_SP, 'Stopped', '0')
```
...
df = df %>% mutate(TRAV_SP = na_if(TRAV_SP, "Stopped"))
```
...
df  %>% 
    ...
    summarise(correlation = cor(df$TRAV_SP, df$AGE, use = "complete.obs"))
```
